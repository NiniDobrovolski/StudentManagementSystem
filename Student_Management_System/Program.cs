...
while (loop) //Loop to handle user interactions
{
    ...
    if (answer == "1" || answer.ToLower() == "add new student")
    {
        try
        {
    ...
    else if (answer == "2" || answer.ToLower() == "view all students")
    {
        //Viewing all students
        sms.ViewAllStudents();
        goto begin;
    }
    else if (answer == "3" || answer.ToLower() == "find student by roll number")
    {
        try
        {
            ...
            int rn;
            bool isValid = int.TryParse(Console.ReadLine(), out rn); //Parsing roll number input
    ...
    else if (answer == "4" || answer.ToLower() == "upgrade student's grade")
    {
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        sms.UpgradeGrade(sms.FindStudent(rollNumber), rollNumber, newGrade);
                        Console.WriteLine("Grade Changed Successfully!"); goto begin;
                    } //Upgrading grade if all conditions are met
                }
            }
        }
        catch (FormatException ex) { Console.WriteLine("Invalid input. Please enter a valid integer for roll number."); } //Handling format exception for roll number 
    ...
    else if (answer == "5" || answer.ToLower() == "view all lecturers")
    {
        sms.ViewLecturers();
        goto begin;
    }
    else if (answer == "6" || answer.ToLower() == "exit")
    {
        //Exiting the program
